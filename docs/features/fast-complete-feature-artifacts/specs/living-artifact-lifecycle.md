...
Success postconditions: the root package describes confirmed behavior, applicable detailed flows
state their success/failure guarantees, and implemented outcomes link to concrete verification.
Candidates remain unapproved. A structural PASS alone does not establish these semantic guarantees.
...
3. `spec.md` records the current state, intended change, resulting behavior, preserved behavior,
   and acceptance scenarios, reviewing applicable failures, permissions, invariants, and interactions.
4. `design.md` records the affected current architecture, resulting responsibilities and flow,
   technical delta, constraints, and risk-triggered details. Relevant quality targets come from
   evidence and link to a design decision and a suitable verification method.
5. When a real decision remains, Kapelle explains current behavior and the proposal before links,
   recommends an evidence-backed option with its downside and
...
8. The developer approves the slice and implementation proceeds through the existing workflow.
   Existing coverage is reused; only basic or concretely important tests are added early.
9. `verify` writes the remaining tests, reuses early coverage, and compares code, tests, contracts,
   diagrams, and project guidance with the human package, including detailed scenario evidence.
...
- At steps 3–4, if a missing quality target blocks a safe decision, it remains explicitly unknown
  and is explained to the developer; no invented target or implementation approval is produced.
- At step 8, a failed important early check prevents claiming that behavior passed. The failing
  outcome is corrected or explicitly deferred under the validation policy; final PASS remains blocked.
- At step 9, missing scenario evidence remains unverified. Updating prose does not turn an
  unexecuted check into PASS, and reusing an early test does not require authoring it again.
