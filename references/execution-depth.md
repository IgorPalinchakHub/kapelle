...
Kapelle no longer changes the number of SDLC stages by size. Every normal feature starts with one
bounded high-level map plus one walking skeleton and repeats `amend -> approve -> implement` for
requested increments before `verify`.

Complexity changes only local effort:

- low uncertainty: main agent, concise documents, no optional subagent;
