...
6. Sequential TDD is the implementation default. Agent Teams require configuration, runtime support,
   safe file partitioning, and explicit user approval.
7. Project guidance is provider-neutral. Kapelle records evidence but never prescribes storage, search,
   rule codes, tools, agents, or commands.
8. Skips are explicit and confirmed.
9. No git operations are performed by the harness.
10. Project-specific behavior belongs in native project capabilities, not in core stages.
