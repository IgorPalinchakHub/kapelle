...
4. A project architecture-rules skill is provider-neutral; Kapelle does not prescribe its name
   or storage.
...
8. Only basic or concretely justified critical tests are added during production implementation;
   reuse existing coverage first (see `references/human-control.md`).
9. `verify` writes all remaining tests and one feature-level verification record.
